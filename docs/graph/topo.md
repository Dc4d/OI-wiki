...
DFS遍历整个DAG，每次DFS结束（到达一个不能继续DFS需要回溯的点）时，把这个终点作为拓扑排序的末尾结点，依次向前填写整个拓扑顺序，DFS得出的拓扑顺序是可行顺序中随机的一个，无法保证字典序最大/最小。
...
在DFS过程中每到达一个出度为0的点，那么这个点一定可以作为拓扑顺序的最后一个结点（因为没有其他结点依赖它）
